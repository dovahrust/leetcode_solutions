...
impl Solution {
    ...
    pub fn shortest_alternating_paths(n: i32, red_edges: Vec<Vec<i32>>, blue_edges: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        let mut heap: BinaryHeap<(Reverse<u8>, Colors, u8)> = BinaryHeap::with_capacity(2 * n as usize);
        let (st_cost, st_node) = (0, 0);
        heap.push((Reverse(st_cost), Colors::Red, st_node));
        heap.push((Reverse(st_cost), Colors::Blue, st_node));
        costs[0] = (0, 0);

        while let Some((Reverse(cost), u_color, u)) = heap.pop() {
            match u_color {
                Colors::Red => {
                    if cost > costs[u as usize].0 {
                        continue;
                    }
                    for &(v, v_color) in &adj[u as usize] {
                        if v_color != Colors::Red && costs[v as usize].1 > cost + 1 {
                            costs[v as usize].1 = cost + 1;
                            heap.push((Reverse(cost + 1), Colors::Blue, v));
                        }
                    }
                },
                Colors::Blue => {
                    if cost > costs[u as usize].1 {
                        continue;
                    }
                    for &(v, v_color) in &adj[u as usize] {
                        if v_color != Colors::Blue && costs[v as usize].0 > cost + 1 {
                            costs[v as usize].0 = cost + 1;
                            heap.push((Reverse(cost + 1), Colors::Red, v));
                        }
                    }
                }
            } 
